...
Идея: не изобретаем свой tool-use цикл, а оркестрируем уже существующие
...
# без предела зависший процесс держит вызывающего вечно.
...
async def run_claude_code(prompt: str, *, cwd: str | None = None, task_id: str | None = None,
    ...
    задачи делает вызывающая сторона, здесь просто прокидывается флагом.
    """
...
async def run_codex(prompt: str, *, cwd: str | None = None, task_id: str | None = None,
    ...
    effort: low | medium | high — выбор по сложности задачи делает вызывающая сторона.
    ...
    # даже если модель сгенерировала правильный код. Вызывающая сторона отвечает
    # за то, чтобы cwd указывал на безопасную (изолированную) копию — здесь мы
    # разрешаем запись явно, раз уж вызвали именно этот раннер.
